...
def test_removes_metadata_and_trailing_payload(image_format):
    ...
    assert MARKER in content.getvalue()
...
@pytest.mark.parametrize('profile', [b'invalid-profile', b'x' * (1024 * 1024 + 1)])
...
def test_rejects_truncated_jpeg():
    ...
    with pytest.raises(ValidationError):
        sanitize_image(ContentFile(content.getvalue()[:-20], name='truncated.jpg'), LIMIT)
