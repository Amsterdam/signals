...
class SpanCreationTestCase(APITestCase):
    ...
    def test_span_creation(self):
        """
        Test if the app succesfully creates spans by creating a custom trace and performing a query inside it.
        """
        ...
        self.assertTrue(len(self.test_span_processor.spans) > 0, "No spans were created")

        # Find the span for the database query
        db_span = next((span for span in self.test_span_processor.spans if span.name == "SELECT"), None)
        ...
        root_span = next((span for span in self.test_span_processor.spans if span.name == "test_span"), None)
        self.assertIsNotNone(root_span, "No root span found")

        if root_span:
            self.assertEqual(root_span.resource.attributes["service.name"], settings.MONITOR_SERVICE_NAME)
    ...
    def test_response_hook(self):
        ...
        request_span = next(
            (span for span in self.test_span_processor.spans
                if 'http.method' in span.attributes and span.attributes['http.method'] == "GET"
            ), None
        )
    ...
    def test_exception_logging_with_traceback(self):
        ...
        with self.assertRaises(ValueError), self.assertLogs(level='ERROR') as log_context:
            client.get('/test-exception/')

        self.assertTrue(any("ValueError: Test exception" in log for log in log_context.output))
        self.assertTrue(any("Traceback (most recent call last):" in log for log in log_context.output))
