...
DEFAULT_ALLOWED_HOSTS: str = (
    "api.data.amsterdam.nl,acc.api.data.amsterdam.nl,localhost,127.0.0.1"
)
...
if SILK_ENABLED:
    ...
    SILK_AUTHENTICATION_ENABLED: bool = (
        os.getenv("SILK_AUTHENTICATION_ENABLED") in TRUE_VALUES
    )
...
ADMIN_ENABLE_LOCAL_LOGIN: bool = (
    os.getenv("ADMIN_ENABLE_LOCAL_LOGIN", False) in TRUE_VALUES
)
...
CELERY_TASK_ALWAYS_EAGER: bool = (
    os.getenv("CELERY_TASK_ALWAYS_EAGER", False) in TRUE_VALUES
)
CELERY_TASK_DEFAULT_PRIORITY = (
    5  # Make sure that all task are scheduled with a default priority of '5'
)
...
O365_ACTUALLY_SEND_IN_DEBUG: bool = (
    os.getenv("O365_ACTUALLY_SEND_IN_DEBUG", False) in TRUE_VALUES
)
...
REST_FRAMEWORK: dict[str, int | str | list[str] | dict[str, str]] = dict(
    ...
    DEFAULT_THROTTLE_RATES={
        ...
        "anon-my_signals": os.getenv(
            "MY_SIGNALS_ANON_REPORTER_THROTTLE_RATE", "5/quarter"
        ),  # noqa:  Quarter was added only for this throttling class
    },
...
SIGMAX_SEND_FAIL_TIMEOUT_MINUTES: str | int = os.getenv(
    "SIGMAX_SEND_FAIL_TIMEOUT_MINUTES", 60 * 24
)  # noqa Default is 24hrs.
SIGMAX_END_STATE_IS_AFGEHANDELD: bool = (
    os.getenv("SIGMAX_END_STATE_IS_AFGEHANDELD", False) in TRUE_VALUES
)
...
ML_TOOL_ENDPOINT: str = os.getenv(
...
SEARCH: dict[str, int | dict[str, str]] = {
    ...
    "CONNECTION": {
        ...
        "STATUS_MESSAGE_INDEX": os.getenv(
            "ELASTICSEARCH_STATUS_MESSAGE_INDEX", "status_messages"
        ),
    },
...
PDOK_LOCATIESERVER_SUGGEST_ENDPOINT: str = os.getenv(
    "PDOK_LOCATIESERVER_SUGGEST_ENDPOINT",
    "https://api.pdok.nl/bzk/locatieserver/search/v3_1/suggest",
)
DEFAULT_PDOK_MUNICIPALITIES: list[str] = os.getenv(
    "DEFAULT_PDOK_MUNICIPALITIES", "Amsterdam,Amstelveen,Weesp,Ouder-Amstel"
).split(",")
...
ENABLE_PUBLIC_GEO_SIGNAL_ENDPOINT: bool = (
    os.getenv("ENABLE_PUBLIC_GEO_SIGNAL_ENDPOINT", False) in TRUE_VALUES
)

# Allow 'invalid' address as unverified
ALLOW_INVALID_ADDRESS_AS_UNVERIFIED: bool = (
    os.getenv("ALLOW_INVALID_ADDRESS_AS_UNVERIFIED", False) in TRUE_VALUES
)
...
MARKDOWNX_MARKDOWNIFY_FUNCTION: str = (
    "signals.apps.email_integrations.utils.markdownx_md"  # noqa Renders markdown as HTML using Mistune
)
MARKDOWNX_URLS_PATH: str = (
    "/signals/markdownx/markdownify/"  # The url path that Signals has for markdownx
)
...
SIGNAL_API_CONTEXT_GEOGRAPHY_RADIUS: int = int(
    os.getenv("SIGNAL_API_CONTEXT_GEOGRAPHY_RADIUS", "50")
)  # Radius in meters

SIGNAL_API_CONTEXT_GEOGRAPHY_CREATED_DELTA_WEEKS: int = int(
    os.getenv("SIGNAL_API_CONTEXT_GEOGRAPHY_CREATED_DELTA_WEEKS", "12")
)  # Created gte X weeks ago

SIGNALS_API_GEO_PAGINATE_BY: int = int(os.getenv("SIGNALS_API_GEO_PAGINATE_BY", "4000"))

TEST_LOGIN: str = os.getenv("TEST_LOGIN", "signals.admin@example.com")

# Feature Flags
FEATURE_FLAGS: dict[str, bool] = {
    "API_DETERMINE_STADSDEEL_ENABLED": os.getenv(
        "API_DETERMINE_STADSDEEL_ENABLED", True
    )
    in TRUE_VALUES,
    "API_TRANSFORM_SOURCE_BASED_ON_REPORTER": os.getenv(
        "API_TRANSFORM_SOURCE_BASED_ON_REPORTER", True
    )
    in TRUE_VALUES,
    "AUTOMATICALLY_CREATE_CHILD_SIGNALS_PER_CONTAINER": os.getenv(
        "AUTOMATICALLY_CREATE_CHILD_SIGNALS_PER_CONTAINER", False
    )
    in TRUE_VALUES,  # noqa
    "AUTOMATICALLY_CREATE_CHILD_SIGNALS_PER_EIKENPROCESSIERUPS_TREE": os.getenv(
        "AUTOMATICALLY_CREATE_CHILD_SIGNALS_PER_EIKENPROCESSIERUPS_TREE", False
    )
    in TRUE_VALUES,  # noqa
    "API_USE_QUESTIONNAIRES_APP_FOR_FEEDBACK": os.getenv(
        "API_USE_QUESTIONNAIRES_APP_FOR_FEEDBACK", False
    )
    in TRUE_VALUES,  # noqa
    # Enable/disable system mail for Feedback Received
    "SYSTEM_MAIL_FEEDBACK_RECEIVED_ENABLED": os.getenv(
        "SYSTEM_MAIL_FEEDBACK_RECEIVED_ENABLED", False
    )
    in TRUE_VALUES,  # noqa
    # Enable/disable status mail for Handled after negative feedback
    "REPORTER_MAIL_HANDLED_NEGATIVE_CONTACT_ENABLED": os.getenv(
        "REPORTER_MAIL_HANDLED_NEGATIVE_CONTACT_ENABLED", False
    )
    in TRUE_VALUES,  # noqa
    # Enable/disable only mail when Feedback allows_contact is True
    "REPORTER_MAIL_CONTACT_FEEDBACK_ALLOWS_CONTACT_ENABLED": os.getenv(
    ...
    "DELETE_SIGNALS_IN_STATE_X_AFTER_PERIOD_Y_ENABLED": os.getenv(
    ...
    "DSL_RUN_ROUTING_EXPRESSIONS_ON_UPDATES": os.getenv(
        "DSL_RUN_ROUTING_EXPRESSIONS_ON_UPDATES", False
    )
    in TRUE_VALUES,
    # Enable/Disable showing employee details in the PDF export
    "SHOW_EMPLOYEE_DETAILS_IN_PDF": os.getenv("SHOW_EMPLOYEE_DETAILS_IN_PDF", True)
    in TRUE_VALUES,
