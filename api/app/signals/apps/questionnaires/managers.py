...
class QuestionManager(models.Manager):
    ...
    def retrieve_reachable_for_question_graph(self, question_graph):
        """
        Makes use of the util function to retrieve the Question QuerySet

        :param question_graph:
        :return QuerySet:
        """
        from signals.apps.questionnaires.models.utils import retrieve_reachable_questions
        return retrieve_reachable_questions(question_graph)

    def get_reachable_from_question_graph(self, question_graph):
        """
        Deprecated function, use `retrieve_reachable_for_question_graph` instead, this function will be removed from
        the code A.S.A.P.

        :param question_graph:
        :return QuerySet:
        """
        return self.retrieve_reachable_for_question_graph(question_graph=question_graph)
