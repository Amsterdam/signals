...
class PrivateSignalSerializerDetail(HALSerializer, AddressValidationMixin):
    ...
    def validate(self, attrs):
        if 'category_assignment' in attrs:
            ...
            # A user who does not have the SIA_ALL_CATEGORIES permission (and thus has limited
            # access to categories) should not be able to move a signal to a category he doesn't
            # have access to.
            if not user.has_perm('signals.' + permissions.SIA_ALL_CATEGORIES) and \
                    not user.has_perm('signals.' + attrs['category_assignment'][
                        'category'].permission.codename):
                raise PermissionDenied(
