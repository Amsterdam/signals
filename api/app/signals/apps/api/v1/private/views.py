...
class PrivateSignalViewSetBase:
    ...
    If user has SIA_ALL_CATEGORIES permission, the user has access to all signals (besides whatever
    restrictions are placed on the user permissions with the Django REST permission_classes). If the
    user does not have the SIA_ALL_CATEGORIES permission, we make sure the user only receives
    signals from categories she has access to.
    ...
    def signals_queryset(self):
        ...
        if user and user.has_perm('signals.' + permissions.SIA_ALL_CATEGORIES):
            return self.all_signals.all()

        return self.all_signals.filter(
            category_assignment__category__permission__in=user.user_permissions.all()
        )
