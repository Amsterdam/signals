...
def _set_service_level(apps, schema_editor):
    """
    Set a service level for each sub category.
    """
    ServiceLevelAgreement = apps.get_model('signals', 'ServiceLevelAgreement')
    Category = apps.get_model('signals', 'Category')

    for sub_slug, entry in SERVICE_LEVELS.items():
        n_days, use_calendar_days = entry
        sub_cat = Category.objects.get(parent__isnull=False, slug=sub_slug)
        ServiceLevelAgreement.objects.create(
            category=sub_cat, n_days=n_days, use_calendar_days=use_calendar_days)

    sub_slugs = SERVICE_LEVELS.keys()
