...
def send_mail_reporter_status_changed_split(signal, *args, **kwargs):
    mail_actions = MailActions()
    return mail_actions.apply(signal_id=signal.pk)


def send_mail_reporter_status_changed_ingepland(signal, *args, **kwargs):
    mail_actions = MailActions()
    return mail_actions.apply(signal_id=signal.pk)


def send_mail_reporter_status_changed_heropend(signal, *args, **kwargs):
    mail_actions = MailActions()
    return mail_actions.apply(signal_id=signal.pk)
