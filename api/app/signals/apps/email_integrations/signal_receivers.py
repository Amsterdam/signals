...
@receiver(create_child, dispatch_uid='email_integrations_create_child')
def create_child_handler(sender, signal_obj, **kwargs):
    # Note these are a copy of what happens in `create_initial_handler` if
    # applicable (so no mail to reporter, but various departments will
...
def update_status_handler(sender, signal_obj, status, prev_status, **kwargs):
    ...
    tasks.send_mail_apptimize.delay(pk=signal_obj.id)
    tasks.send_mail_reporter_status_changed_split(signal_pk=signal_obj.id, status_pk=status.id)
    tasks.send_mail_reporter_status_changed_ingepland(signal_pk=signal_obj.id, status_pk=status.id)
