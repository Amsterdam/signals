...
from tests.test import SIAAllCategoriesUserMixin, SignalsBaseApiTestCase
...
class TestAuthAPIEndpoints(SignalsBaseApiTestCase, SIAAllCategoriesUserMixin):
    endpoints = [
        '/signals/auth/signal/',
        '/signals/auth/status/',
    ...
    def setUp(self):
        ...
        self.client.force_authenticate(user=self.sia_all_categories_user)
...
class TestAuthAPIEndpointsPOST(TestAPIEndpointsBase):
    ...
    def test_post_status_push_to_sigmax_permission_denied(self):
        ...
        user.user_permissions.add(Permission.objects.get(codename=permissions.SIA_ALL_CATEGORIES))
