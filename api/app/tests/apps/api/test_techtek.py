...
class TestTechtekFlow(SignalsBaseApiTestCase):
    ...
    def setUp(self):
        ...
        # Give the Techtek user the correct permissions (as backoffice user, the tests in
        # test_category_permissions.py verify the case in which a Techtek user has only category
        # permissions)
        required_permissions = Permission.objects.filter(codename__in=[
            permissions.SIA_WRITE,
            permissions.SIA_READ,
            permissions.SIA_ALL_CATEGORIES,
        ])
